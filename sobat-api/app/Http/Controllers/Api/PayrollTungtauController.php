...
class PayrollTungtauController extends Controller
{
    ...
    {
        $user = auth()->user();
        $query = PayrollTungtau::with('employee');
    ...
    public function import(Request $request)
    {
        ...
        try {
            ...
            Log::error('Tungtau Payroll Import Error: ' . $e->getMessage());
    ...
    public function saveImport(Request $request)
    {
        ...
        try {
            ...
            foreach ($request->rows as $index => $row) {
                ...
                $existing = PayrollTungtau::where('employee_id', $employee->id)
            ...
            Log::error('Tungtau Payroll Save Error: ' . $e->getMessage());
            return response()->json(['message' => 'Error: ' . $e->getMessage()], 500);
        }
    }
    ...
    {
        $payroll = PayrollTungtau::with('employee')->findOrFail($id);
    ...
    public function updateStatus(Request $request, $id)
    {
        ...
        $payroll = PayrollTungtau::findOrFail($id);
    ...
    public function generateSlip($id)
    {
        try {
            ...
            $pdf = PDF::loadView('payslips.tungtau', [
                'payroll' => $payroll,
                'aiMessage' => $aiMessage
            ]);
            
            $filename = 'payslip_tungtau_' . str_replace(' ', '_', $payroll->employee->full_name) . '_' . $payroll->period . '.pdf';

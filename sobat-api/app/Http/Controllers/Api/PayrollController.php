    ...
    public function index(Request $request)
    {
                ...
                foreach ($payrollModels as $modelClass => $type) {
                    ...
                    foreach ($records as $record) {
                        ...
                        $data = [
                            ...
                            'division_type_label' => $type,
                            'status' => $record->status === 'draft' ? 'pending' : $record->status,
                        ];

                        if ($modelClass === \App\Models\Payroll::class) {
                            $data['basic_salary'] = round((float) $record->basic_salary, 0);
                            $data['allowances'] = round((float) ($record->allowances ?? 0), 0);
                            $data['overtime_pay'] = round((float) ($record->overtime_pay ?? 0), 0);
                            $data['deductions'] = round((float) ($record->total_deductions ?? 0), 0);
                            $data['bpjs_health'] = round((float) ($record->bpjs_kesehatan ?? 0), 0);
                            $data['bpjs_employment'] = round((float) ($record->bpjs_ketenagakerjaan ?? 0), 0);
                            $data['tax'] = round((float) ($record->pph21 ?? 0), 0);
                            $data['gross_salary'] = round((float) $record->gross_salary, 0);
                            $data['net_salary'] = round((float) $record->net_salary, 0);
        ...
        return response()->json([
            'data' => $payrolls->map(function ($payroll) {
                ...
                if (isset($payroll->details) && is_array($payroll->details)) {
                    $details = $payroll->details;
                    ...
                    $formatRate = function ($amount, $defaultLabel) use ($daysPresent) {
                        if ($daysPresent > 0 && $amount > 0 && fmod($amount, $daysPresent) == 0) {
                            $rate = $amount / $daysPresent;
                            if ($rate >= 1000) { // Reasonable daily rate threshold
                                return $defaultLabel . ' (Rp ' . number_format($rate, 0, ',', '.') . ' /hari)';
                            }
                        }
                        return $defaultLabel;
                    };
                    ...
                    if (!empty($allowancesMap)) {
                        $data['allowances'] = $allowancesMap;
                    }

                    if (($details['overtime_hours'] ?? 0) > 0) {
                        $data['allowances'] = $data['allowances'] ?? [];
                        $data['allowances']['Lembur'] = [
                            'amount' => $payroll->overtime_pay ?? 0,
                            'hours' => $details['overtime_hours'],
                        ];
                    }

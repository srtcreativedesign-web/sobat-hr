    ...
    public function simulateImport(Request $request)
    {
        ...
        try {
            ...
            for ($row = $startDataRow; $row <= $highestRow; $row++) {
                ...
                // FORCE: If hours are zero, amount MUST be zero, no matter what Excel says
                if ($overtimeHours <= 0) {
                    $overtimeAmount = 0;
                } elseif ($overtimeAmount <= 0 && $overtimeRate > 0) {
                    // Fallback: Calculate overtime amount from rate × hours if amount not mapped
                    $overtimeAmount = $overtimeRate * $overtimeHours;
                }
                
                // FORCE: If days present are zero, mandatory amount MUST be zero
                if ($daysPresent <= 0) {
                    $mandatoryOvertimeAmount = 0;
                } elseif ($mandatoryOvertimeAmount <= 0 && $mandatoryOvertimeRate > 0) {

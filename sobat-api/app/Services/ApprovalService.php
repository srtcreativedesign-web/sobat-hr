...
class ApprovalService
{
            ...
            if (isset($approverIds[0])) {
                ...
                if ($approver && $approver->user) {
                    try {
                        ...
                        $this->sendFcmToUser($approver->user, 'Persetujuan Diperlukan', $approver->user->employee->full_name.' mengajukan '.($request->type ?? 'permohonan').'. Mohon ditinjau.');
                    } catch (\Exception $e) {
            ...
            if ($nextPendingStep) {
                ...
                if ($nextPendingStep->approver && $nextPendingStep->approver->user) {
                    try {
                        ...
                        $this->sendFcmToUser($nextPendingStep->approver->user, 'Persetujuan Diperlukan', 'Tahap approval selanjutnya menunggu persetujuan Anda.');
                    } catch (\Exception $e) {
    ...
    private function sendFcmToUser($user, string $title, string $body): void
    {
        try {
            ...
            Log::error("FCM send failed to user {$user->id}: {$e->getMessage()}");
        }
    }
}

...
<head>
    <meta charset="utf-8">
    <title>Slip Gaji Wrapping</title>
    <style>
        body {
...
<body>
    ...
    <table class="info-table">
        <tr>
            ...
            <td style="width: 100px; text-align: right;">: SAL-WRA-{{ $payroll->id }}</td>
        </tr>
        <tr>
    ...
    <table class="details-table">
        ...
        @if($payroll->training_salary > 0)
        <tr>
            <td>Gaji Training</td>
            <td class="amount">Rp {{ number_format($payroll->training_salary, 0, ',', '.') }}</td>
        </tr>
        @endif
        ...
        @if($payroll->target_koli > 0)
        <tr>
            <td>Target Koli</td>
            <td class="amount">Rp {{ number_format($payroll->target_koli, 0, ',', '.') }}</td>
        </tr>
        @endif
        
        @if($payroll->fee_aksesoris > 0)
        <tr>
            <td>Fee Aksesoris</td>
            <td class="amount">Rp {{ number_format($payroll->fee_aksesoris, 0, ',', '.') }}</td>
        </tr>
        @endif
        ...
        @if($payroll->adj_bpjs > 0)
        <tr>
            <td>Adj BPJS (Refund)</td>
            <td class="amount">Rp {{ number_format($payroll->adj_bpjs, 0, ',', '.') }}</td>
        </tr>
        @endif
         ...
         <tr>
            ...
            <td class="amount" style="font-weight: bold;">Rp {{ number_format($payroll->total_salary_gross > 0 ? $payroll->total_salary_gross : ($payroll->net_salary + $payroll->deduction_total), 0, ',', '.') }}</td>
        </tr>
    </table>

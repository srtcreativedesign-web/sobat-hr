        ...
        <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end pointer-events-none">
            ...
            {isOpen && (
                <div className="mb-4 bg-white w-80 md:w-96 rounded-2xl shadow-2xl border border-gray-100 overflow-hidden pointer-events-auto animate-fade-in-up flex flex-col max-h-[500px]">
                    {/* Header */}
                    <div className="bg-[#419cc3] p-4 flex justify-between items-center text-white">
                        <div className="flex items-center gap-3">
                            <div className="w-8 h-8 bg-[#89b4e1] rounded-full flex items-center justify-center text-[#419cc3] font-bold text-sm">
                                AI
                            </div>
                            <div>
                        ...
                        {messages.map((msg, idx) => (
                            <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                <div className={`max-w-[85%] rounded-2xl px-4 py-2.5 text-sm shadow-sm ${msg.role === 'user'
                                    ? 'bg-[#419cc3] text-white rounded-br-none'
                    ...
                    <div className="p-3 bg-white border-t border-gray-100">
                        <form onSubmit={handleSubmit} className="relative">
                            <input
                                ...
                                className="w-full pl-4 pr-12 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#89b4e1] focus:border-[#419cc3] outline-none transition-all text-sm"
                            />
                            <button
                                type="submit"
                                disabled={!input.trim() || isLoading}
                                className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 text-[#419cc3] bg-[#89b4e1] rounded-lg hover:bg-[#8fdad2] disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                            >
            ...
            <button
                ...
                className="pointer-events-auto bg-[#419cc3] text-white p-4 rounded-full shadow-xl hover:shadow-2xl hover:scale-110 active:scale-95 transition-all duration-300 group"
            >
                {isOpen ? (
                    ...
                    <div className="relative">
                        ...
                        <span className="absolute -top-1 -right-1 flex h-3 w-3">
                            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#89b4e1] opacity-75"></span>
                            <span className="relative inline-flex rounded-full h-3 w-3 bg-[#89b4e1]"></span>
                        </span>
                    </div>
                )}
            </button>

        ...
        <div className="glass-card p-8 animate-fade-in-up mt-8">
            <div className="flex items-center justify-between mb-6">
                ...
                <button
                    ...
                    className="px-4 py-2 bg-[#1C3ECA] text-white text-sm font-semibold rounded-lg hover:bg-[#143d24] transition-colors flex items-center gap-2"
                >
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-2">
                                            <input
                                                ...
                                                readOnly
                                                value={link}
                                                className="flex-1 bg-white border border-gray-200 text-gray-600 text-sm rounded-lg px-3 py-2 focus:outline-none focus:border-[#1C3ECA] w-64"
                                            />
                                            <button
                                                onClick={() => handleCopyLink(invite.token, invite.id)}
                                                className={`p-2 rounded-lg transition-all ${copiedId === invite.id
                                                    ? 'bg-green-100 text-green-700'
                                                    : 'bg-gray-100 text-gray-600 hover:bg-[#1C3ECA] hover:text-white'
                                                    }`}
                                                title="Copy Link"
                                            >
                                                {copiedId === invite.id ? (

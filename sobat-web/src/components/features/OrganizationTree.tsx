...
const TreeNode = ({ node, onEdit, onAddChild, onAddSibling, onDelete, onSelect, onAddParent }: TreeNodeProps) => {
    ...
    const getTypeBadge = (type: string) => {
        switch (type) {
            case 'headquarters': return 'bg-[#419cc3] text-white';
        ...
        <li className={lineClass}>
            ...
            >
                <div className="flex flex-col items-center gap-2">
                    ...
                    <h3 className="font-bold text-[#419cc3] text-lg">{node.name}</h3>
                    ...
                    <div className="flex items-center gap-2 mt-2 pt-2 border-t border-gray-100 opacity-0 group-hover:opacity-100 transition-opacity justify-center" onClick={(e) => e.stopPropagation()}>
                        ...
                        <button
                            ...
                            className="p-1.5 text-white bg-[#89b4e1] hover:bg-[#8fdad2] rounded-full transition-colors"
                            title="Add Child (Down)"
                        >
                            <svg className="w-4 h-4 text-[#419cc3]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 14l-7 7m0 0l-7-7m7 7V3" /></svg>
                        </button>
                        <button
                        ...
                        <button
                            onClick={() => onEdit(node)}
                            className="p-1.5 text-gray-500 hover:text-[#419cc3] hover:bg-gray-100 rounded-full transition-colors"
                            title="Edit"
                        >
        ...
        <div className="relative w-full h-[600px] bg-gray-50 rounded-xl overflow-hidden border border-gray-200">
            ...
            <div className="absolute bottom-4 right-4 z-50 flex flex-col gap-2 bg-white p-2 rounded-lg shadow-lg border border-gray-100">
                <button
                    ...
                    className="p-2 hover:bg-gray-100 rounded-lg text-[#419cc3] transition-colors"
                    title="Zoom In"
                >

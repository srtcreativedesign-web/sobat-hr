...
export default function ManageDepartmentsPage() {
    ...
    const fetchDepartments = async () => {
        ...
        try {
            ...
            Swal.fire('Error', 'Gagal mengambil data departemen', 'error');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleDelete = async (id: number) => {
        ...
        if (result.isConfirmed) {
            try {
                ...
                Swal.fire('Terhapus!', 'Departemen berhasil dihapus.', 'success');
                fetchDepartments();
            } catch (error: any) {
                console.error('Error deleting department:', error);
                if (error.response?.status === 409) {
                    Swal.fire('Gagal', 'Tidak dapat menghapus departemen yang memiliki divisi.', 'error');
                } else {
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm">
                        <thead className="bg-gray-50 border-b border-gray-100">
                            <tr>
                                <th className="px-6 py-4 font-semibold text-gray-700">Nama Departemen</th>
                            ...
                            {isLoading ? (
                                ...
                                departments.map((department) => (
                                    <tr key={department.id} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 text-gray-600">{department.description || '-'}</td>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50">
                    <div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden">
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Nama Departemen <span className="text-red-500">*</span></label>
                                <input
                                    type="text"
                                    required
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Kode Departemen</label>
                                <input
                                    type="text"
                                    className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all"
                                    value={formData.code}
